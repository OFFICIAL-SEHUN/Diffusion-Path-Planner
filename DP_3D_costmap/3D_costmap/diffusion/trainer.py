...
Trains model with Slope + CoT 2-channel input to generate energy-efficient paths.
"""

class Trainer:
    """
    Diffusion Model Trainer for Slope + CoT Path Planning
    ...
    def train(self):
        ...
        wandb.init(
            project="diffusion-slope-cot-path-planning",  # Updated project name
            config=self.config,
            name=run_name 
        )
        ...
        for epoch in range(epochs):
            ...
            for costmaps, paths in pbar:
                ...
                # Costmaps should be [B, 2, H, W] (2-channel: Slope + CoT)
            ...
            if (epoch + 1) % log_interval == 0:
                ...
                with torch.no_grad():
                    ...
                    # Create visualization (use CoT channel for display)
                    fig, ax = plt.subplots(figsize=(6, 6))
                    
                    # Plot CoT costmap (Channel 1)
                    costmap_np = test_costmap.squeeze().cpu().numpy()  # [2, H, W]
                    cot_channel = costmap_np[1]  # Channel 1: CoT
                    ax.imshow(cot_channel, cmap='hot', origin='lower', vmin=0, vmax=1)
                    ax.set_title(f"Epoch {epoch+1} - Generated Path on CoT Map")
                    
                    # Plot generated path
                    h, w = cot_channel.shape
